...
class MessageHandler:
    ...
    def run(self):
        ...
        while True:
            queue_event = self.queue.get(
                block=True
            )  # waits for incoming queue_event objects
            for event_handler in self.event_listeners:
                ...
                if (
                    not queue_event
                ):  # if eventhandler returns Null, then no further message handling
                    break

    def add_event_handler(self, name, priority, event_handler):

        self.mutex.acquire()  # avoid thread interfearence
        self.event_listeners.append(EventListener(name, priority, event_handler))
    ...
    def add_query_handler(self, name, priority, query_handler):
        ...
        self.mutex.acquire()  # avoid thread interfearence
        self.query_handlers.append(QueryHandler(name, priority, query_handler))
        # release the mutex lock
        self.mutex.release()
    ...
    def query(self, query):
        ...
        try:
            if (
                "query_start_page" in query.params
                and query.params["query_start_page"] >= 0
            ):
        ...
        if query.unlimited_nr_of_results:
            for query_handler in self.query_handlers:
                ...
                while not all_received:
                    this_res = query_handler.query_handler(
                        query, defaults.MAX_QUERY_SIZE * query_start_size + 1
                    )
                    # there are more result
                    all_received = (
                        len(this_res) <= defaults.MAX_QUERY_SIZE * query_start_size
                    )
                    query_start_size *= (
                        2  # double the block size for another loop, if needed
                    )
                res += this_res
            return res
        else:
            for query_handler in self.query_handlers:
                ...
                if defaults.MAX_QUERY_SIZE * (query_start_page + 1) - len(res) < 0:
                    break
                res += query_handler.query_handler(
                    query,
                    defaults.MAX_QUERY_SIZE * (query_start_page + 1) - len(res) + 1,
                )  # max. + 1
            # returns one result more as dictated by MAX_QUERY_SIZE to indicate that there are some more results
            return res[
                defaults.MAX_QUERY_SIZE
                * query_start_page : defaults.MAX_QUERY_SIZE
                * (query_start_page + 1)
                + 1
            ]
