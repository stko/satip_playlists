...
class SplPlugin(SplThread):
    ...
    def __init__(self, modref):
        ...
        self.config = JsonStorage(
            ...
            "config.json",
            {
                ...
                "actual_settings": {
                    "www_root_dir": os.path.realpath(
                        os.path.join(self.program_dir, "../../../static")
                    )
                },
            },
        )
        ...
        parser.add_argument(
            "-p", "--port", default=server_config["port"], help="the server port"
        )
        parser.add_argument(
            "-s",
            "--secure",
            action="store_true",
            default=server_config["secure"],
            help="use secure https: and wss:",
        )
        parser.add_argument(
            "-c",
            "--credentials",
            default=server_config["credentials"],
        ...
        parser.add_argument(
            ...
            "--browser",
            action="store_true",
            default=server_config["openbrowser"],
    ...
    def event_listener(self, queue_event):
        ...
        if queue_event.type == defaults.MSG_SOCKET_MSG:
            message = {
                "type": queue_event.data["type"],
                "config": queue_event.data["config"],
            }
    ...
    def _run(self):
        ...
        try:
            ...
            if self.args.secure:
                print("initialized secure https server at port %d" % (self.args.port))
