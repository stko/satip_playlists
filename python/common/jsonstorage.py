...
import os
import json
import simple_tv_logger
from directorymapper import DirectoryMapper

logger = simple_tv_logger.getLogger(__name__)
...
class JsonStorage:
    ...
    def __init__(self, module_name, storage_type, filename, default):
        ...
        try:
            with DirectoryMapper.open(
                self.module_name, self.storage_type, self.file_name
            ) as json_file:
    ...
    def save(self):
        ...
        try:
            with DirectoryMapper.open(
                self.module_name, self.storage_type, self.file_name, "w"
            ) as outfile:
                try:
                    json.dump(
                        self.config,
            ...
            with DirectoryMapper.open(
                ...
                try:
                    ...
                    logger.warning(
                        "couldn't write unsorted config file {0}: {1}".format(
                            self.file_name, str(ex2)
                        )
                    )
        except Exception as ex:
            logger.warning(
                "couldn't write config file {0}: {1}".format(self.file_name, str(ex))
            )
